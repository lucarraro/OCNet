...
output: 
    bookdown::html_document2:
      ...
      fig_caption: true
vignette: >
  %\VignetteIndexEntry{An introduction to OCNet}
...
OCN <- create_OCN(30, 20, outletPos = 1, seed=1)
OCN <- aggregate_OCN(landscape_OCN(OCN), thrA = 3)
par(mfrow = c(1, 3), mai = c(0, 0, 0.2, 0.2))
draw_simple_OCN(OCN, thrADraw = 3, main = "Optimal Channel Network")
draw_elev3D_OCN(OCN, drawRiver = FALSE, addColorbar = FALSE, expand = 0.2, main = "Elevation")
draw_thematic_OCN(OCN$AG$streamOrder, OCN, discreteLevels = TRUE, colPalette = rainbow(4), 
                  main = "Strahler stream order")
...
OCNwe <- create_OCN(20, 20, outletPos = 3, cellsize = 500, seed=1)
par(mai=c(0,0,0,0))
plot(OCNwe)
...
par(mai=c(0,0,0,0.5))
draw_elev3D_OCN(OCNwe, drawRiver = FALSE)  # or plot(OCNwe, which="elev3D", drawRiver = FALSE)
...
plot(pop[OCNwe$RN$outlet, ], type = "l", ylim = c(0, 1.05*K[OCNwe$RN$outlet]), col = "red", 
     xlab = "Time", ylab = "Population", lwd = 2, main = "Evolution of local pop. size")
...
plot(colSums(pop), type = "l", xlab = "Time", ylab = "Population", lwd = 2, 
     ylim = c(0, 1.05*sum(K)), main = "Evolution of metapop. size")
...
draw_simple_OCN(peano0, main = "Iteration: 0 - Lattice size: 2x2")

peano1 <- create_peano(1)
draw_simple_OCN(peano1, main = "Iteration: 1 - Lattice size: 4x4")

peano2 <- create_peano(2)
draw_simple_OCN(peano2, main = "Iteration: 2 - Lattice size: 8x8")

peano3 <- create_peano(3)
draw_simple_OCN(peano3, main = "Iteration: 3 - Lattice size: 16x16")

peano4 <- create_peano(4)
draw_simple_OCN(peano4, main = "Iteration: 4 - Lattice size: 32x32")

peano5 <- create_peano(5)
draw_simple_OCN(peano5, main = "Iteration: 5 - Lattice size: 64x64")
...
plot.SpatialStreamNetwork(ssnOCN, "upDist", breaktype = "user", brks = seq(0,14000,2000), 
                          ...
                          main = "Distance from outlet of observation points [m]")
